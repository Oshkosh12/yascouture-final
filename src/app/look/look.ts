...
export class Look {
  ...
  // Use @Input() only if data comes from parent, otherwise use local array
  @Input() 
  set lookData(data: any[]) {
    if (data && data.length > 0) {
      // Use data from parent component
      this._lookData = data.map(item => ({
        ...item,
        showSecondImage: false
      }));
    } else {
      // Use default data if no input provided
      this._lookData = this.defaultLookData.map(item => ({
        ...item,
        showSecondImage: false
      }));
    }
  ...
  private _lookData: any[] = [];

  // Default data array
  private defaultLookData = [
      {
        name: 'LOOK 1',
        img: 'assets/looks/Look-27-600x900.jpeg.webp',
        text: 'Soft Sage Green mini dress with Delicate Crystal and Bead Embellishments...',
  ...
  ngOnInit() {
    // If no input provided, use default data
    if (!this._lookData || this._lookData.length === 0) {
      this.lookData = []; // This will trigger the setter
    }
  }

  private updateAllPictures() {
    this.allPictures = [];
    this.lookData.forEach(product => {
      this.allPictures.push(product.img);
      if (product.img2) {
        this.allPictures.push(product.img2);
      }
    });
  }

  toggleImage(index: number, event: Event) {
    event.stopPropagation();
    if (this.lookData[index].img2) {
      this.lookData[index].showSecondImage = !this.lookData[index].showSecondImage;
    }
  }
  ...
  getCurrentImage(product: any): string {
    return product.showSecondImage && product.img2 ? product.img2 : product.img;
  }
}